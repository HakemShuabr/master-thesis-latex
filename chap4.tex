...
\begin{figure}[ht]
    ...
    \caption[The decision curve analysis (DCA) for the NC vs. CI classification task]{The decision curve analysis (DCA) for the NC vs. CI classification task. \hl{The DCA shows the net benefit of the calibrated LR model compared to ``treat all'' and ``treat none'' strategies in both external validation datasets.}}
    \label{fig:nc_ci_dca}
\end{figure}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=1.0\textwidth]{figures/XAI/impaired_classification_adni_true_negative_waterfall.png}
...
\begin{figure}[ht]
    ...
    \caption[Feature importance in the counterfactual explanation for the NC vs. CI classification task]{Feature importance in the counterfactual explanation for the NC vs. CI classification task. \hl{The bar chart shows the frequency of changes in each feature to generate counterfactual examples that reverse the model's prediction from cognitive impairment to normal cognition.}}
...
\begin{figure}[ht]
    ...
    \caption[Feature importance in the counterfactual explanation for the CIND vs. dementia classification task]{Feature importance in the counterfactual explanation for the CIND vs. dementia classification task. \hl{The bar chart shows the frequency of changes in each feature to generate counterfactual examples that reverse the model's prediction from dementia to CIND.}}
...
\begin{figure}[ht]
    ...
    \caption[The decision curve analysis (DCA) for the future NC vs. CI prediction task]{The decision curve analysis (DCA) for the future NC vs. CI prediction task. \hl{The DCA shows the net benefit of the calibrated LR model compared to ``treat all'' and ``treat none'' strategies in both external validation datasets.}}
...
\begin{figure}[ht]
    ...
    \caption[The decision curve analysis (DCA) for the future CIND vs. dementia prediction task]{The decision curve analysis (DCA) for the future CIND vs. dementia prediction task. \hl{The DCA shows the net benefit of the calibrated LR model compared to ``treat all'' and ``treat none'' strategies in both external validation datasets.}}
    \label{fig:future_cind_dementia_dca}
\end{figure}

\hl{The external validation results indicate that the developed models for predicting future cognitive status showed promising generalizability within the evaluated datasets.} Both prediction models achieved high performance in the ADNI study, especially in detecting cases with high risk (sensitivity). The models also scored high accuracy in the NACC cross-center validation compared to the internal CV. In addition, the models presented excellent calibration and clinical utility to accurately estimate the risk of CI and dementia, especially in the NACC cross-center validation. These results \hl{support} the second objective of the study, \hl{indicating} that accessible baseline data can be used to effectively predict the risk of cognitive impairment up to five years in the future.
...
\begin{figure}[ht]
    ...
    \caption[Feature importance in the counterfactual explanation for the future NC vs. CI prediction task]{Feature importance in the counterfactual explanation for the future NC vs. CI prediction task. \hl{The bar chart shows the frequency of changes in each feature to generate counterfactual examples that reverse the model's prediction from cognitive impairment to normal cognition.}}
...
\begin{figure}[ht]
    ...
    \caption[Feature importance in the counterfactual explanation for the future CIND vs. dementia prediction task]{Feature importance in the counterfactual explanation for the future CIND vs. dementia prediction task. \hl{The bar chart shows the frequency of changes in each feature to generate counterfactual examples that reverse the model's prediction from dementia to CIND.}}
...
\begin{figure}[ht]
    ...
    \caption[The decision curve analysis (DCA) of the NACC cross-center validation for the AD vs. non-AD classification task]{The decision curve analysis (DCA) of the NACC cross-center validation for the AD vs. non-AD classification task. \hl{The DCA shows the net benefit of the calibrated LR model compared to ``treat all'' and ``treat none'' strategies in the NACC cross-center validation dataset.}}
...
\hl{This chapter has presented and analyzed the results of the five classification and prediction tasks, covering feature selection, data resampling, model fine-tuning and selection, external validation, and explainability. The implications of these results are discussed in the following chapter.}