...
\begin{table}[!ht]
    ...
    \hline
        ...
        ROS & \makecell{0.831 \\ \hl{{\small (p = 0.010)}}} & 0.885 & 0.781 & 0.213 & 0.293 \\ \hline
        RUS & \makecell{0.831 \\ \hl{{\small (p = 0.057)}}} & 0.\textbf{892} & 0.773 & 0.158 & 0.259 \\ \hline
        SMOTE & \makecell{0.831 \\ \hl{{\small (p = 0.004)}}} & 0.881 & 0.785 & 0.207 & 0.280 \\ \hline
        \textbf{ENN + KMeansSMOTE} & \makecell{\textbf{0.832} \\ \hl{{\small (p = 0.026)}}} & 0.862 & \textbf{0.804} & \textbf{0.005} & \textbf{0.059} \\ \hline
    \end{tabular}
...
\begin{table}[!ht]
    ...
    \newcolumntype{C}{>{\centering\arraybackslash}X}
        ...
        $11,613$ \\
        \hline
        \multicolumn{5}{p{400pt}}{\textsuperscript{1}Gain is calculated as the ratio of the selected model’s throughput to the benchmark model’s throughput. Values indicate the factor by which training and prediction speed have improved. Gain is not presented when the selected model is the benchmark model.}
    \end{tabularx}
\end{table}
...
The generalizability of the selected LR models was assessed for the two current cognitive status classification tasks. This was done by evaluating their performance and clinical utility in the NACC cross-center holdout test dataset and the independent Alzheimer's Disease Neuroimaging Initiative (\acrshort{ADNI}) dataset. A detailed description of the validation methodology is provided in section \ref{sec:model-validation}.
...
\begin{table}[!ht]
    ...
    \newcolumntype{C}{>{\centering\arraybackslash}X}
        \begin{tabularx}{\textwidth}{|c|c|C|C|}
            ...
            \hline
            AUC & \makecell{0.844 \\ \hl{{\small (0.843--0.846)}}} & \makecell{0.810 \\ \hl{{\small (0.802--0.818)}}} & \makecell{0.825 \\ \hl{{\small (0.807--0.842)}}} \\ \hline
            ...
            \hline
            \multicolumn{4}{p{400pt}}{Sensitivity refers to the recall of the CI class, while Specificity refers to the recall of the NC class.}
        \end{tabularx}
\end{table}
...
\begin{table}[!ht]
    ...
    \newcolumntype{C}{>{\centering\arraybackslash}X}
        \begin{tabularx}{\textwidth}{|c|c|C|C|}
            ...
            \textbf{Metric} & \textbf{Internal CV} & \textbf{NACC Cross-center Validation} & \textbf{ADNI External Validation} \\
            \hline
            AUC & \makecell{0.828 \\ \hl{{\small (0.821--0.835)}}} & \makecell{0.828 \\ \hl{{\small (0.819--0.837)}}} & \makecell{0.841 \\ \hl{{\small (0.822--0.861)}}} \\ \hline
            ...
            \hline
            \multicolumn{4}{p{400pt}}{Sensitivity refers to the recall of the dementia class, while Specificity refers to the recall of the CIND class.}
        \end{tabularx}
\end{table}
...
\begin{table}[!ht]
    ...
    \hline
        ...
        ROS & \makecell{0.749 \\ \hl{{\small (p = 0.972)}}} & 0.853 & 0.658 & 0.272 & 0.385 \\ \hline
        RUS & \makecell{0.752 \\ \hl{{\small (p = 0.175)}}} & 0.845 & 0.669 & 0.229 & 0.365 \\ \hline
        SMOTE & \makecell{0.751 \\ \hl{{\small (p = 0.135)}}} & 0.860 & 0.656 & 0.267 & 0.446 \\ \hline
        \textbf{ENN + BorderlineSMOTE} & \makecell{\textbf{0.752} \\ \hl{{\small (p = 0.343)}}} & 0.812 & \textbf{0.697} & \textbf{0.018} & \textbf{0.158} \\ \hline
    \end{tabular}
...
\begin{table}[!ht]
    ...
    \hline
        ...
        ROS & \makecell{0.715 \\ \hl{{\small (p = 0.101)}}} & 0.754 & 0.678 & 0.244 & 0.469 \\ \hline
        RUS & \makecell{0.726 \\ \hl{{\small (p = 0.014)}}} & 0.723 & \textbf{0.728} & 0.250 & 0.470 \\ \hline
        SMOTE & \makecell{0.715 \\ \hl{{\small (p = 0.314)}}} & 0.758 & 0.675 & 0.238 & 0.432 \\ \hline
        \textbf{ENN + KMeansSMOTE} & \makecell{\textbf{0.730} \\ \hl{{\small (p = 0.022)}}} & \textbf{0.776} & 0.688 & \textbf{0.034} & \textbf{0.161} \\ \hline
    \end{tabular}
...
\newcolumntype{C}{>{\centering\arraybackslash}X}
    \begin{tabularx}{\textwidth}{|C|C|C|C|C|}
        ...
        $20,074$ $(1.28\times)$ \\
        \hline
        \multicolumn{5}{p{400pt}}{\textsuperscript{1}Gain is calculated as the ratio of the selected model’s throughput to the benchmark model’s throughput. Values indicate the factor by which training and prediction speed have improved.}
    \end{tabularx}
\end{table}
...
\subsection{Model Performance in External Validation}
The two selected LR models for the prediction of cognitive status 5 years in the future were tested using two separate datasets to assess their generalizability. First, the models were tested on the NACC holdout test dataset (which represents 20\% of the original data) to evaluate their performance on unseen data from different research centers. In addition, an independent external validation was performed using the ADNI dataset. Moreover, the results of the models' calibration and clinical utility evaluation in both external validation datasets were presented. The details of the validation method are described in section \ref{sec:model-validation}.
...
\begin{table}[!ht]
    ...
    \newcolumntype{C}{>{\centering\arraybackslash}X}
        \begin{tabularx}{\textwidth}{|c|c|C|C|}
            ...
            \textbf{Metric} & \textbf{Internal CV} & \textbf{NACC Cross-center Validation} & \textbf{ADNI External Validation} \\
            \hline
            AUC & \makecell{0.772 \\ \hl{{\small (0.769--0.776)}}} & \makecell{0.790 \\ \hl{{\small (0.774--0.806)}}} & \makecell{0.778 \\ \hl{{\small (0.750--0.803)}}} \\ \hline
...
\begin{table}[!ht]
    ...
    \newcolumntype{C}{>{\centering\arraybackslash}X}
        \begin{tabularx}{\textwidth}{|c|c|C|C|}
            ...
            \textbf{Metric} & \textbf{Internal CV} & \textbf{NACC Cross-center Validation} & \textbf{ADNI External Validation} \\
            \hline
            AUC & \makecell{0.759 \\ \hl{{\small (0.751--0.768)}}} & \makecell{0.805 \\ \hl{{\small (0.784--0.828)}}} & \makecell{0.780 \\ \hl{{\small (0.740--0.819)}}} \\ \hline
...
\begin{table}[!ht]
    ...
    \hline
        ...
        ROS & \makecell{0.673 \\ \hl{{\small (p = 0.125)}}} & 0.666 & 0.681 & 0.317 & 0.503 \\ \hline
        RUS & \makecell{0.674 \\ \hl{{\small (p = 0.157)}}} & 0.639 & \textbf{0.712} & 0.306 & 0.522 \\ \hline
        SMOTE & \makecell{0.674 \\ \hl{{\small (p = 0.011)}}} & 0.677 & 0.671 & 0.315 & 0.491 \\ \hline
        \textbf{ENN + KMeansSMOTE} & \makecell{\textbf{0.710} \\ \hl{{\small (p = 0.001)}}} & \textbf{0.729} & 0.692 & \textbf{0.021} & \textbf{0.125} \\ \hline
    \end{tabular}
...
\newcolumntype{C}{>{\centering\arraybackslash}X}
    \begin{tabularx}{\textwidth}{|C|C|C|C|}
        ...
        $7,466$ \\
        \hline
        \multicolumn{4}{p{400pt}}{\textsuperscript{1}Gain is calculated as the ratio of the selected model’s throughput to the benchmark model’s throughput. Values indicate the factor by which training and prediction speed have improved. Gain is not presented when the selected model is the benchmark model.}
    \end{tabularx}
\end{table}
...
\subsection{Model Performance in External Validation}
The generalizability of the selected LR model for the AD vs. non-AD classification task was tested on the NACC holdout test dataset, which contains unseen data from different research centers. The details of the validation method are provided in section \ref{sec:model-validation}.
...
\begin{table}[!ht]
    ...
    \begin{tabular}{|c|c|c|}
        ...
        \hline
        AUC & \makecell{0.702 \\ \hl{{\small (0.698--0.706)}}} & \makecell{0.670 \\ \hl{{\small (0.658--0.681)}}} \\ \hline
