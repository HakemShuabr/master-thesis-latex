...
% Packages and commands added by myself
...
\sethlcolor{yellow}
\usepackage{pifont}
\newcommand{\yesmark}{\ding{52}} % to create a tick mark
\newcommand{\nomark}{\ding{56}} % to create a cross mark
\newcommand{\partialmark}{\rule[0.4ex]{1.4ex}{1.4pt}} % to create a square for partial mark
\newcommand{\rh}[1]{\rotatebox[origin=c]{90}{\hspace{4pt}\textbf{#1}\hspace{4pt}}} % to rotate table header text
\newcommand{\twenty}[1]{} % to fix sorting in bibliography
\renewcommand{\hl}[1]{#1} % to turn off the text highlight by \hl command 
\widowpenalty=10000 % to avoid widow lines in the thesis. Widow lines are the last line of a paragraph that appears alone at the top of a page.
\clubpenalty=10000 % to avoid club lines in the thesis. Club lines are the first line of a paragraph that appears alone at the bottom of a page.
...
\include{chap2} % Literature Review
\include{chap3} % Methodology
\include{chap4} % Results and Analysis
\include{chap5} % Discussion
